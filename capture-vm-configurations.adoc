...
---
= Acquisire le configurazioni VM correnti
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
. Nel riquadro VMware, selezionare *Plan and Deploy* (Pianificazione e distribuzione).
+
image:screenshot-choose-method.png["Una schermata dei diversi metodi che è possibile utilizzare per raccogliere le informazioni di configurazione correnti della macchina virtuale."]
...
.. Esegui lo script di raccolta dati scaricato e specifica l'opzione di raccolta dati per le statistiche VM "quotidiane" o "orarie".
+
...
+
Per ulteriori informazioni, vedere il capitolo "parametri della riga di comando" nella https://resources.robware.net/resources/prod/RVTools.pdf["Documentazione di RVTools"^] .
...
link:launch-onboarding-advisor.html["Creare un piano di distribuzione utilizzando il Migration Advisor"].
